...
const mockedGetSupplierOrders = vi.mocked(supplierService.getSupplierOrders);

const backendOrdersResponse = {
  fornecedor: 'RTech Distribuidora 1 Ltda',
...
const waitForOrdersToLoad = async () => {
  await screen.findByText('PC0001');
};
...
describe('SupplierInfoModal', () => {
  beforeEach(() => {
    ...
    mockedGetSupplierOrders.mockResolvedValue(backendOrdersResponse as never);
  });

  // --- Rendering ---

  it('renders supplier name and code', async () => {
    renderModal();
  ...
  it('renders supplier category', async () => {
    renderModal();
    await waitForOrdersToLoad();
    expect(screen.getByText('Materiais de Solda')).toBeInTheDocument();
  });

  it('renders supplier city and region', async () => {
    renderModal();
    await waitForOrdersToLoad();
  ...
  it('renders total orders count', async () => {
    renderModal();
    await waitForOrdersToLoad();
    await waitFor(() => expect(mockedGetSupplierOrders).toHaveBeenCalledWith('FOR001'));
    expect(screen.getByText('5')).toBeInTheDocument();
  });

  it('renders total delays count', async () => {
    renderModal();
    await waitForOrdersToLoad();
    expect(screen.getByText('2')).toBeInTheDocument();
  });

  it('renders all previous orders rows', async () => {
    renderModal();
    await waitForOrdersToLoad();
  ...
  it('renders active status dot when supplier is active', async () => {
    renderModal();
    await waitForOrdersToLoad();
  ...
  it('renders inactive status dot when supplier is inactive', async () => {
  ...
  it('shows check icon when delay rate is below 20%', async () => {
  ...
  it('shows warning icon when delay rate is between 20% and 40%', async () => {
  ...
  it('shows danger icon when delay rate is 40% or more', async () => {
  ...
  it('shows check icon when supplier has zero orders', async () => {
  ...
  it('filters orders by material name via search', async () => {
    renderModal();
    await waitForOrdersToLoad();
  ...
  it('filters orders by order code via search', async () => {
    renderModal();
    await waitForOrdersToLoad();
    const input = screen.getByPlaceholderText('Buscar pedido ou material...');
  ...
  it('filters orders by project code via search', async () => {
    renderModal();
    await waitForOrdersToLoad();
    const input = screen.getByPlaceholderText('Buscar pedido ou material...');
  ...
  it('filters orders by project via dropdown', async () => {
    renderModal();
    await waitForOrdersToLoad();
  ...
  it('shows all orders when dropdown is reset to empty', async () => {
    renderModal();
    await waitForOrdersToLoad();
    const select = screen.getByLabelText('Filtrar por projeto');
  ...
  it('applies search and dropdown filter simultaneously', async () => {
    renderModal();
    await waitForOrdersToLoad();
    const select = screen.getByLabelText('Filtrar por projeto');
    fireEvent.change(select, { target: { value: 'PRJ001' } });
